...
- How should the cron discover and connect to the job state database — direct connection or via the Rebalance Job Service API?
- Should the cron run as a standalone scheduled task (e.g., Kubernetes CronJob) or as an in-process scheduled function within an existing service?
- How should the staleness threshold configuration be managed (environment variable, config service, database setting)?
- What happens if the cron process itself crashes mid-run — are partial transitions safe?
...
Considerations for engineering team:
- Should staleness detection use `updated_at` or `started_at` as the reference timestamp, or both?
- How should the cron handle database connection failures or timeouts during the detection query?
- Should job transitions be batched or processed individually?
- What locking mechanism (if any) prevents concurrent cron instances from processing the same jobs?
...
Considerations for engineering team:
- Are existing `status`, `updated_at`, and `started_at` columns sufficient for staleness detection, or are schema additions needed?
- Should a `resolved_by` or `failure_reason` column be added to distinguish cron-resolved failures from organic failures?
- Is an index needed on `(status, updated_at)` to support efficient staleness queries at scale?
...
Considerations for engineering team:
- What is the expected volume of in-progress jobs at any given time, and how does this affect query performance?
- Should the detection query use pagination or streaming to avoid loading large result sets into memory?
- How can the cron's database queries be isolated from production read/write traffic (e.g., read replica)?
...
Considerations for engineering team:
- What if a job is legitimately long-running and gets incorrectly marked as stale? How should the threshold account for this?
- What is the blast radius if the cron has a bug that marks active jobs as failed?
- How should the cron behave during database maintenance windows or failovers?
- Are there race conditions between the cron's staleness check and a job completing normally?
...
Considerations for engineering team:
- What is the minimal viable version (e.g., cron + detection + marking, without metrics)?
- Can the cron be deployed in dry-run mode first (detect and log, but do not transition)?
- What dependencies must be in place before the cron can be enabled in production?
- How should rollback work if the cron causes unexpected issues?
